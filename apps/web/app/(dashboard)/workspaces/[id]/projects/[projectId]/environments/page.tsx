...
export default function EnvironmentsPage({ params }: { params: Promise<{ id: string; projectId: string }> }) {
  ...
  const remove = useMutation({ mutationFn: () => api.deleteEnvironment(projectId, deleting?.id ?? ""), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["environments", projectId] }); setDeleting(null); } });
  ...
  return (
    <section className="grid gap-6">
      ...
      <Card>
        {(workspace.isLoading || project.isLoading || environments.isLoading) && <SkeletonRows rows={5} columns={4} />}
        ...
        {!!environments.data?.length && <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left text-slate-500 dark:border-slate-800"><th className="py-2">Name</th><th>Slug</th><th>Created</th><th className="text-right">Actions</th></tr></thead><tbody>{environments.data.map((environment) => <tr className="border-b last:border-0 dark:border-slate-800" key={environment.id}><td className="py-3 font-medium">{environment.name}</td><td>{environment.slug}</td><td>{formatDate(environment.created_at)}</td><td className="flex justify-end gap-2"><Button asChild variant="outline" size="sm"><Link href={`/secrets?workspace=${id}&project=${projectId}&environment=${environment.id}`}>View secrets</Link></Button><Button type="button" variant="danger" size="sm" onClick={() => setDeleting(environment)}>Delete</Button></td></tr>)}</tbody></table></div>}
      </Card>
      ...
      <Modal title="Delete Environment" open={deleting !== null} onClose={() => setDeleting(null)}><div className="grid gap-4"><p className="text-sm text-slate-600 dark:text-slate-300">Delete environment {deleting?.name}?</p>{remove.isError && <p className="text-sm text-red-700">Environment could not be deleted.</p>}<Button type="button" variant="danger" disabled={remove.isPending} onClick={() => remove.mutate()}>{remove.isPending ? "Deleting..." : "Confirm delete"}</Button></div></Modal>
    </section>
  );
}